...
public class InvTweaksGuiSettings extends InvTweaksGuiSettingsAbstract {
    ...
    public void initGui() {
        ...
        InvTweaksGuiTooltipButton shortcutsBtn = new InvTweaksGuiTooltipButton(
                ID_SHORTCUTS,
                p.getX(),
                p.getY(),
                130,
                20,
                computeBooleanButtonLabel(InvTweaksConfig.PROP_ENABLE_SHORTCUTS, labelShortcuts),
                StatCollector.translateToLocal("invtweaks.settings.shortcuts.tooltip"));
        ...
        if (shortcuts.equals(InvTweaksConfig.VALUE_CI_COMPATIBILITY)) {
            ...
            shortcutsBtn.setTooltip(
                    shortcutsBtn.getTooltip() + "\n("
                            + StatCollector.translateToLocal("invtweaks.settings.disableci.tooltip")
                            + ")");
        }

        moveToButtonCoords(i++, p);
        InvTweaksGuiTooltipButton beforeBreakBtn = new InvTweaksGuiTooltipButton(
                ID_BEFORE_BREAK,
                p.getX(),
                p.getY(),
                computeBooleanButtonLabel(InvTweaksConfig.PROP_AUTO_REFILL_BEFORE_BREAK, labelAutoRefillBeforeBreak),
                StatCollector.translateToLocal("invtweaks.settings.beforebreak.tooltip"));
        controlList.add(beforeBreakBtn);

        moveToButtonCoords(i++, p);
        InvTweaksGuiTooltipButton autoRefillBtn = new InvTweaksGuiTooltipButton(
                ID_AUTO_REFILL,
                p.getX(),
                p.getY(),
                computeBooleanButtonLabel(InvTweaksConfig.PROP_ENABLE_AUTO_REFILL, labelAutoRefill),
                StatCollector.translateToLocal("invtweaks.settings.autorefill.tooltip"));
        ...
        controlList.add(
                new InvTweaksGuiTooltipButton(
                        ...
                        p.getY(),
                        labelMoreOptions,
                        StatCollector.translateToLocal("invtweaks.settings.moreoptions.tooltip")));
        ...
        moveToButtonCoords(i++, p);
        InvTweaksGuiTooltipButton middleClickBtn = new InvTweaksGuiTooltipButton(
                ID_MIDDLE_CLICK,
                p.getX(),
                p.getY(),
                computeBooleanButtonLabel(InvTweaksConfig.PROP_ENABLE_MIDDLE_CLICK, labelMiddleClick),
                StatCollector.translateToLocal("invtweaks.settings.middleclick.tooltip"));
        ...
        if (middleClick.equals(InvTweaksConfig.VALUE_CI_COMPATIBILITY)) {
            ...
            middleClickBtn.setTooltip(
                    middleClickBtn.getTooltip() + "\n("
                            + StatCollector.translateToLocal("invtweaks.settings.disableci.tooltip"));
        }

        // Check if links to files are supported, if not disable the buttons
        if (!Desktop.isDesktopSupported()) {
            for (val guiButton : controlList) {
