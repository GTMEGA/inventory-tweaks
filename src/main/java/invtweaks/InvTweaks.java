...
public class InvTweaks extends InvTweaksObfuscation {
    ...
    private PriorityQueue<TickScheduledTask> scheduledTasks = new PriorityQueue<TickScheduledTask>(
            16,
            new TickScheduledTask.TaskComparator());
    ...
    public void onItemPickup() {
        ...
        try {
            InvTweaksContainerSectionManager containerMgr = new InvTweaksContainerSectionManager(
                    mc,
            ...
            if (currentSlot != -1) {
                ...
                List<IItemTreeItem> items = tree
                        .getItems(Item.itemRegistry.getNameForObject(stack.getItem()), stack.getItemDamage());
    ...
    public void logInGame(String message, boolean alreadyTranslated) {
        String formattedMsg = buildlogString(
                Level.INFO,
    ...
    private void handleConfigSwitch() {
        ...
        if (isSortingShortcutDown() && switchMapping != null) {
            ...
            if (newRuleset != null) {
                logInGame(
                        String.format(StatCollector.translateToLocal("invtweaks.loadconfig.enabled"), newRuleset),
                        true);
                ...
                sortingKeyPressedDate = currentTime;
                    ...
                    && sortingKeyPressedDate != Integer.MAX_VALUE) {
                        ...
                        if (previousRuleset != null && newRuleset != null && !previousRuleset.equals(newRuleset)) {
                            logInGame(
                                    String.format(
                                            StatCollector.translateToLocal("invtweaks.loadconfig.enabled"),
                                            newRuleset),
                                    true);
    ...
    private void handleAutoRefill() {
        ...
        String currentStackId = (currentStack == null) ? null
                : Item.itemRegistry.getNameForObject(currentStack.getItem());
                            ...
                            && (getCurrentScreen() == null || // Filter open inventory or other window
                                    isGuiEditSign(getCurrentScreen()))) { // TODO: This should be more expandable on
                                        ...
                                        if (config.isAutoRefillEnabled(storedStackId, storedStackDamage)) {
                                            try {
                                                cfgManager.getAutoRefillHandler()
                ...
                if (!chestAlgorithmButtonDown) {
                    ...
                    if (((IInvTweaksContainer) container).invtweaks$validChest()) {
                        ...
                        if (ContainerSection.CHEST.equals(target)) {
                            ...
                            if (ContainerSection.INVENTORY_HOTBAR.equals(target)
                ...
                if (!isValidChest) {
                    controlList.add(
                            new InvTweaksGuiSettingsButton(
                                    ...
                                    guiContainer.guiTop + 5,
                                    w,
                                    h,
                                    "...",
                                    StatCollector.translateToLocal("invtweaks.button.settings.tooltip"),
                ...
                // Chest buttons
                    ...
                    if (!config.getProperty(InvTweaksConfig.PROP_SHOW_CHEST_BUTTONS).equals("false")) {
                        ...
                        GuiButton button = new InvTweaksGuiSortingButton(
                                ...
                                h,
                                "h",
                                StatCollector.translateToLocal("invtweaks.button.chest3.tooltip"),
                        ...
                        button = new InvTweaksGuiSortingButton(
                                ...
                                h,
                                "v",
                                StatCollector.translateToLocal("invtweaks.button.chest2.tooltip"),
                                SortingMethod.VERTICAL,
                                rowSize,
                                customTextureAvailable);
                        controlList.add(button);
                        ...
                        button = new InvTweaksGuiSortingButton(
                                ...
                                h,
                                "s",
                                StatCollector.translateToLocal("invtweaks.button.chest1.tooltip"),
        ...
        if (e != null && e.getStackTrace() != null && e.getStackTrace().length > 0) {
            ...
            if (exceptionLine != null && exceptionLine.getFileName() != null) {
                return buildlogString(level, message) + ": "
                        + e.getMessage()
                        + " (l"
                        + exceptionLine.getLineNumber()
                        + " in "
                        + exceptionLine.getFileName().replace("InvTweaks", "")
                        + ")";
            } else {
